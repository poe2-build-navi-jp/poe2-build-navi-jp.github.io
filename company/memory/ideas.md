# コンテンツ・施策アイデアのバックログ

2026-09-08時点で `data/builds.json` を実際に検査して見つかった手薄箇所と、
今後の施策候補。優先度は上から順ではなく、着手時にユーザーと相談して選ぶ。

## data/builds.json の手薄箇所（実データ検証済み）

- **評価指標が全ビルドでnull**: `beginnerRating` / `damageRating` /
  `defenseRating` / `mappingRating` / `bossRating` の5項目が、8ビルド全てで
  未設定（null）。評価基準（何を1〜5点等でどう採点するか）の設計自体が
  サイト全体の課題。中途半端な主観評価より、まず採点基準のフォーマットを
  決めてから埋めるべき。
  - **2026-09-26解決**: 採点基準を設計し `/rating-criteria/` で公開、全11ビルドに
    点数と根拠（`ratingEvidence`）を入力。火力・耐久・周回・ボスは元ガイドの
    長所・短所の記述のみで採点し、記述がない項目は `null`（未評価）のまま。
    未評価が多いのは huntress-companion / witch-minion-infernalist（Maxrollに
    長所・短所欄なし、Mobalyticsは403で取得不可）。追加資料が取れれば埋める。
- **sources（裏付け情報源）の件数にばらつき**: 2026-09-08に手薄だった4ビルド
  （mercenary=1→2、huntress=2→2〈新規源に差し替え〉、warrior=2→3、witch=2→3）
  全てにMaxroll等の2件目・3件目の情報源をWebFetchで直接検証のうえ追加した。
  現在は全ビルドが2件以上（mercenary/huntress=2、warrior/monk/ranger/witch=3、
  sorceress/druid=4）。優先度の高い「1件のみ」ビルドは無くなった。
  - **2026-09-10追記**: このセッション外で新規ビルドが2本追加され
    （mercenary/grenade-gemling、witch/ed-contagion-lich）、両方とも
    情報源1件・`ssf: null`の手薄な状態だった。
  - **2026-09-11解決**: 両方ともMaxrollのガイドをWebFetchで直接検証し、
    2件目の情報源として追加。grenade-gemlingは「SSF Fresh Start
    Leaguestart Viable」と明記、ed-contagion-lichは「ユニークアイテム
    不要」＋SSFキャンペーン所要時間の目安が明記されており、両方とも
    `ssf: true`に更新した。現在、全10ビルドが2件以上の情報源を持つ。
- **ssf（ソロセルフファウンド適性）が半数で未設定**:
  monk, mercenary, huntress, druid の4ビルドで `ssf: null`。
  SSFプレイヤー向けの需要は根強いため、検証して埋める価値がある。
  - mercenary(twister-gemling)は2026-09-08に調査したが未解決のまま。
    Maxrollは早期にユニークアミュレット「The Taming」入手を推奨する一方、
    WebSearchの合成要約は「uniqueアイテム不要でSSF向き」と正反対の主張をしており、
    aoeahのSSF Tier10には掲載なし。poe.ninjaの実データ
    （Runes of Aldur SSFリーグのGemling Legionnaire一覧）で裏を取ろうとしたが、
    JS描画ページのためWebFetchでは中身が取得できなかった。今後もし取り組むなら、
    poe.ninjaをブラウザで直接確認するか、実際のキャラクタービルドURLを個別に
    WebFetchする必要がある。現状は`ssf: null`のまま、weaknessesに
    「情報源間で矛盾があり未確認」という趣旨を記載済み。
  - monk(whirling-assault)は2026-09-08に調査し解決。Maxrollのガイドに
    「ユニークアイテムを必要としない」との明記があり、`ssf: true`に更新した。
  - druid(plant-oracle)は2026-09-08に調査したが、mercenaryと同様に未解決。
    Maxrollのガイドはエンドゲームで複数の重要ユニーク（Atalui's Bloodletting等）
    を挙げており、序盤は安価に始められるとしつつ終盤の完成度はSSFで下がる
    可能性がある。`ssf: null`のまま、weaknessesにその旨を記載済み。
  - 残る未解決はmercenaryとdruidの2件（huntressは明示断定なしだが前向きな
    情報のみでstrengthsに追記済み）。
- **levelingStagesの各段階のnowActions等は充実している一方、Endgame以降の
  比較データ（他ビルドとの相対評価、装備の具体的な価格帯・入手難易度）は
  記事内では扱われていない**（要件からの推測。深掘り時に確認）。

## 施策候補（次の一手）

1. 評価指標（5項目）の採点基準を設計し、8ビルド分を埋める。
2. sourcesが手薄なビルド（mercenary最優先、次点でwarrior/huntress/witch）に
   一次情報・検証済み情報源を追加する。
3. ssf未検証の4ビルドについて、SSFでの成立可否を調査・記載する。
4. 既存8ビルドの「比較記事」（例：初心者おすすめ度ランキング、コスト別比較、
   ボス攻略適性比較）を新設し、単体ビルド記事同士を横断的にリンクさせる
   （SEO的な内部リンク強化にもなる）。
5. ~~`_next/static/` の要否をユーザーに確認し、不要なら削除してリポジトリを整理する。~~
   2026-09-26削除済み。同時にCSS/JSの`?v=`を内容ハッシュへ統一（`tools/sync-asset-versions.mjs`）、
   `package.json`で`npm test`（jsdom込み）を実行可能にした。
6. PoE2 1.0・基本無料化移行（2026年12月11日予定、伝聞・要一次情報確認）に向けた
   特設コンテンツ（新規プレイヤー向け導線、新クラス情報のプレースホルダー等）の
   準備時期を検討する。ただし一次情報の裏付けが取れてから着手する。

## セッション開始フックとの連動

- `.claude/hooks/session-start-tasks.sh` が上記の「評価指標未設定」「sources不足」
  「ssf未検証」を機械的にスコアリングし、セッション開始時に1件ずつ深掘り候補を
  提示する（`decisions.md` 参照）。このアイデアリストは、その提案を実際の作業に
  落とし込む際の参照先として使う。
