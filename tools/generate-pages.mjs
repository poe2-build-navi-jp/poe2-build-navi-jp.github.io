  ...
  <main id="main">
    <section class="build-hero">
      <div class="build-hero-grid">
        <div><p id="build-class" class="eyebrow">${escapeHtml(build.className)} / ${escapeHtml(build.ascendancy)}</p><h1 id="build-name">PoE2 ${escapeHtml(build.name)} ビルド｜${escapeHtml(build.version)}育成</h1><p id="build-skill" class="lead">メインスキル：${escapeHtml(build.mainSkill)}</p><p>現在Lvを入力すると、次に確認するスキル・装備・パッシブを3つに絞ります。</p><p class="status-note" data-build-status="${escapeHtml(build.status)}">${escapeHtml(statusLabels[build.status] || "状態未登録")}</p><p class="status-note">対応環境 ${escapeHtml(build.version)}・<a href="${escapeHtml(site.latestPatchSource)}" target="_blank" rel="noopener noreferrer">最新確認 ${escapeHtml(site.latestPatch)}</a>・最終確認 ${escapeHtml(site.latestPatchCheckedAt)}</p><div class="build-overview"><p><b>おすすめ：</b>${escapeHtml(build.audience)}</p><p><b>弱点：</b>${escapeHtml(build.weaknesses?.[0] || "未登録")}</p><p><b>操作：</b>${escapeHtml(build.difficulty)}　<b>SSF：</b>${ssfLabel(build)}</p></div><div id="fact-grid" class="fact-grid"><div class="fact">対応パッチ：${escapeHtml(build.version)}</div><div class="fact">資料確認日：${escapeHtml(build.updatedAt)}</div></div></div>
