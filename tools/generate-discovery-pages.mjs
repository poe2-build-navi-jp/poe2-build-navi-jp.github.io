...
const bestComparisonRows = builds.map((build) => {
  ...
  return `<tr><th scope="row"><a href="${buildUrl(build)}?level=1#now">${esc(build.name)}</a><small>${esc(build.ascendancy)}</small></th><td>${esc(build.className)}</td><td>${esc(purpose ? `${purpose}候補` : "育成手順掲載")}</td><td>${mapping}</td><td>${boss}</td><td>初心者Tier ${esc(tierByBuildId.get(build.id) ?? "未評価")}</td><td>${starter}</td><td>${defence}</td><td>${esc(build.difficulty)}</td></tr>`;
}).join("");
