...
home = home
  .replace(/<title>.*?<\/title>/, "<title>PoE2ビルドナビ｜初心者向け日本語ビルド・育成ガイド</title>")
  .replace(/<meta name="description" content="[^"]*">/, '<meta name="description" content="PoE2初心者向け日本語ビルドガイド。おすすめビルドを選び、現在Lvを入力すると、Lv1からEndgameまで次にやることが分かります。">')
  .replace(/<meta property="og:title" content="[^"]*">/, '<meta property="og:title" content="PoE2ビルドナビ｜初心者向け日本語ビルド・育成ガイド">')
  ...
  .replace(/<section class="section section-soft" aria-labelledby="quality-title">[\s\S]*?<\/section>/, `<section class="section section-soft" aria-labelledby="quality-title"><div class="section-head"><p class="section-kicker">SEARCH GUIDES</p><h2 id="quality-title">目的から探す</h2><p>検索した悩みから、ビルド詳細のLv入力まで迷わず進めます。</p></div><div class="tool-grid"><a class="tool-card" href="/tier-list/"><span>TIER LIST</span><strong>今の候補を比較</strong><p>初心者向け育成の始めやすさを暫定比較します。</p></a><a class="tool-card" href="/league-starter/"><span>LEAGUE STARTER</span><strong>最初の1体を選ぶ</strong><p>装備依存と操作の注意を確認します。</p></a><a class="tool-card" href="/poe2-1-0/"><span>POE2 1.0</span><strong>正式版情報</strong><p>確定情報と未確認情報を分けて掲載します。</p></a></div></section>`);
...
await writeFile(resolve(root, "tier-list/index.html"), shell({ title: "PoE2 最新ビルドTierリスト｜初心者おすすめ・最強候補まとめ", description: "PoE2 0.5.5の初心者向けビルドを、育成の始めやすさ・操作・構成切替・資料確認状況で暫定比較。各ビルドは現在Lvから育成できます。", path: "/tier-list/", current: "tier", kicker: "TIER LIST / PATCH 0.5.5", h1: "PoE2 最新ビルドTierリスト", intro: "初心者が最初の1体を選ぶための暫定Tierです。強さだけの順位ではなく、Lv1から迷わず進められるかを重視します。", content: tierContent, crumbs: [{ name: "ホーム", path: "/" }, { name: "Tierリスト", path: "/tier-list/" }] }));
...
const oneContent = `<section><h2>PoE2 1.0とは</h2><p>早期アクセスから正式版へ移行する大型更新です。公式サイトで公開された情報を確認し、確定事項と未確定事項を分けて掲載します。</p></section><section><h2>確認できた正式版情報</h2><ul>${one.confirmed.map((item) => `<li>${esc(item)}</li>`).join("")}</ul><p><a href="${esc(one.sourceUrl)}">${esc(one.sourceName)}</a>（確認：${esc(one.checkedAt)}）</p></section><section class="content-action"><h2>まだ断定しない情報</h2><ul>${one.unconfirmed.map((item) => `<li>${esc(item)}</li>`).join("")}</ul></section><section><h2>正式版前にできること</h2><div class="tool-grid"><a class="tool-card" href="/guides/beginner-build/"><span>初心者向け</span><strong>ビルドの選び方</strong><p>操作と育成切替で選びます。</p></a><a class="tool-card" href="/league-starter/"><span>STARTER</span><strong>リーグ開始候補</strong><p>現在の0.5.5候補を確認します。</p></a><a class="tool-card" href="/builds/"><span>BUILDS</span><strong>現在の10ビルド</strong><p>Lv1からの育成手順へ進みます。</p></a></div></section><section><h2>1.0公開後の更新方針</h2><p>公式パッチノート、スキル変更、アセンダンシー、段階別の書面ガイドを再確認してから、Tier・リーグスターター・各ビルドの対応版を更新します。公開前の予想を最強ランキングとして掲載しません。</p></section>`;
...
const noteMap = `# note → POE2ビルドナビ 送客対応表\n\n同じ本文を転載せず、noteでは特徴・選び方を説明し、サイトではLv1からの手順と現在Lv入力を提供します。\n\n| noteテーマ | リンク先 | UTM付きリンク |\n|---|---|---|\n${noteRows.map(([theme, path, campaign]) => `| ${theme} | ${path} | ${absolute(path)}?utm_source=note&utm_medium=referral&utm_campaign=${campaign} |`).join("\n")}\n\nCTA例：\n\n- Lv1から完成までの育成手順はこちら\n- 現在Lvから次にやることを見る\n- 装備・スキル・パッシブまでまとめて確認\n`;
