...
const tierByBuildId = new Map(Object.entries(discovery.tiers).flatMap(([tier, entries]) => entries.map(({ id }) => [id, tier])));
const purposeByBuildId = new Map(bestBuilds.map(({ id, label }) => [id, label]));
const bestComparisonRows = builds.map((build) => {
  const purpose = purposeByBuildId.get(build.id);
...
const bestContent = `<section class="content-action"><h2>結論：目的別のおすすめ候補</h2><p>「最強」は周回・ボス・防御・装備条件で変わります。掲載${builds.length}ビルドの確認済みデータから、目的ごとに1本ずつ候補を選びました。</p></section><div class="purpose-grid">${bestBuildCards}</div>${bestComparison}<section><h2>Tier・リーグスターターとの違い</h2><div class="pros-cons"><section class="info-card"><h3>初心者向けTier</h3><p>Lv1からの育てやすさ、操作、構成切替、資料確認状況を重視します。</p><a href="/tier-list/">初心者向けTierを見る</a></section><section class="info-card"><h3>リーグスターター</h3><p>資産が少ない序盤から育成できるか、装備依存やSSF情報を重視します。</p><a href="/league-starter/">リーグスターターを見る</a></section></div></section><section><h2>選定と更新の方針</h2><ul><li>掲載中かつLv1〜Endgameの主要情報を確認できたビルドだけを対象にします。</li><li>同条件の実測DPS順位は作らず、既存資料で確認できる用途・操作・装備条件を使います。</li><li>パッチ変更時は公式ノートと各ビルドの段階別資料を再確認します。</li></ul><p><a href="/editorial-policy/">編集方針と情報確認方法を見る</a></p></section><section class="related"><h2>ビルドを決めた後</h2><div class="related-links"><a href="/leveling/">別のビルド・現在Lvから探す</a><a href="/classes/">職業から選び直す</a><a href="/builds/">掲載中の全ビルドを見る</a></div></section>`;
const bestEvidence = `<section aria-labelledby="best-evidence-title"><h2 id="best-evidence-title">PoE2で最強のビルドは？比較の根拠</h2><p>すべての用途で最強と断定できるビルドは、この比較では確認できていません。周回・単体戦・操作量・装備条件を分けて候補を選びます。以下は目的別候補で使用している資料です。各ビルド詳細には段階別の手順と出典を掲載しています。</p><ul class="source-list">${bestBuilds.map(({id}) => { const build = byId(id); const source = build.sources[0]; return `<li><a href="${esc(source.url)}" target="_blank" rel="noopener noreferrer">${esc(build.name)}：${esc(source.name)}</a></li>`; }).join('')}</ul><p>当サイトの用途別候補は掲載資料に基づく編集上の案内です。同じ予算・同じ条件の実測順位ではありません。比較後は個別ビルドで現在Lvを入力すると、次にやることを3つ確認できます。</p></section>`;
...
const oneBeginnerCards = oneBeginnerIds.map((id) => {
  const build = byId(id);
  return compactBuildCard(build, "0.5.5の育成手順を見る", discovery.buildTags?.[id] ?? []);
}).join("");
const oneContent = `<section class="content-action"><h2>結論：PoE2 1.0は2026年12月11日公開予定</h2><ul>${one.confirmed.map((item) => `<li>${esc(item)}</li>`).join("")}</ul><p>日付・無料化・Duelistは公式発表で確認済みです。1.0環境のビルド性能はまだ確定していません。</p></section><section><h2>正式リリース日はいつ？</h2><p>公式発表では、Path of Exile 2の正式版1.0は2026年12月11日（PST）に公開予定です。</p><a class="button-secondary" href="/poe2-1-0/release-date/">公開予定日を詳しく見る</a></section><section><h2>正式版は無料で遊べる？</h2><p>公式発表では、正式版1.0は基本プレイ無料のアクションRPGとして公開予定です。価格・対応機種など、このサイトのビルド選択と直接関係しない未確認情報は扱いません。</p><p><a href="${esc(one.sourceUrl)}" target="_blank" rel="noopener noreferrer">${esc(one.sourceName)}の正式版発表</a>（確認：${esc(one.checkedAt)}）</p></section><section><h2>Early Accessから何が変わる？</h2><p>現時点で公式に確認できる変更は、正式版1.0への移行、基本プレイ無料での公開、Duelistの追加です。キャンペーン、スキル数値、既存ビルドの強弱など、公式発表で確定していない内容はここでは断定しません。</p></section><section><h2>新職業Duelistの公式情報</h2><p>Duelistは正式版1.0で追加予定です。スキル・アセンダンシー・既存ビルドへの影響は、正式なパッチノートと段階別資料を確認できるまで推測で掲載しません。</p><div class="section-cta"><a class="button-secondary" href="/poe2-1-0/duelist/">Duelistの確認済み情報</a><a class="button-secondary" href="${esc(one.duelistSourceUrl)}" target="_blank" rel="noopener noreferrer">Duelistの公式発表</a></div></section><section class="content-action"><h2>初心者は何を選べばいい？</h2><p>1.0向けの最強ビルドはまだ選べません。今プレイする場合は、0.5.5で育成手順を確認済みの候補から、遠距離・召喚・耐久・操作量で選んでください。正式版公開後に再確認できたものだけ1.0対応へ切り替えます。</p><div class="section-cta"><a class="button" href="/best-builds/">0.5.5の目的別候補を見る</a><a class="button-secondary" href="/class-check/">4問で職業を絞る</a></div></section><section><h2>1.0を待たず、今から始めてもいい？</h2><p>現行0.5.5を遊びながら基本操作や育成の流れを知りたい場合は、今から始められます。ただし、現在のビルド性能や装備条件が1.0でも同じとは限りません。1.0用のビルドを決め打ちしたい場合は、正式なパッチノートと再確認済みガイドを待ってください。</p></section><section><h2>1.0までに覚えておきたいこと</h2><div class="tool-grid"><a class="tool-card" href="/beginner-guide/"><span>SKILL</span><strong>スキルと育成の基本</strong><p>ビルドを選び、現在Lvの手順へ進む流れを確認します。</p></a><a class="tool-card" href="/guides/gear-upgrade/"><span>GEAR</span><strong>装備更新</strong><p>今の困りごとから交換候補を絞ります。</p></a><a class="tool-card" href="/guides/passive-tree/"><span>PASSIVE</span><strong>パッシブ</strong><p>育成段階に合う原典の確認方法を見ます。</p></a><a class="tool-card" href="/guides/resistance/"><span>DEFENCE</span><strong>耐性</strong><p>死にやすいときの確認順を整理します。</p></a><a class="tool-card" href="/guides/mapping/"><span>ENDGAME</span><strong>Mapping・Endgame</strong><p>キャンペーン後に確認する項目へ進みます。</p></a></div></section><section><h2>現在選べる初心者向けビルド</h2><p>以下は1.0予想ではなく、現行0.5.5でLv1〜Endgameの主要情報を確認済みの候補です。</p><div class="discovery-grid">${oneBeginnerCards}</div></section><section class="content-action"><h2>1.0公開前に断定しない情報</h2><ul>${one.unconfirmed.map((item) => `<li>${esc(item)}</li>`).join("")}</ul></section><section id="one-build-impact"><h2>現在の0.5.5ビルドへの影響</h2><p>掲載中の${builds.length}ビルドは0.5.5向けです。1.0向けに再確認済みのビルドはまだありません。公式パッチノートと段階別資料を確認するまで、0.5.5の手順を1.0対応とは表示しません。</p></section><section><h2>よくある質問</h2><details><summary>PoE2 1.0はいつ？</summary><p>2026年12月11日（PST）に公開予定です。</p></details><details><summary>正式版は無料？</summary><p>公式発表では、1.0は基本プレイ無料で公開予定です。</p></details><details><summary>0.5.5のビルドを1.0でもそのまま使える？</summary><p>まだ確認できません。正式な変更内容と段階別資料を確認してから更新します。</p></details></section><section><h2>更新履歴</h2><ul><li>2026-09-25：初心者向け判断、今から始める場合の注意、基礎ガイド、現行ビルド導線を追加</li><li>2026-09-24：検索意図を整理し、確定情報・未確定情報・FAQを分離</li><li>2026-09-23：正式版・基本プレイ無料化・Duelistの公式情報を更新</li></ul></section><section><h2>1.0公開後の更新方針</h2><p>公式パッチノート、スキル変更、アセンダンシー、段階別の書面ガイドを再確認してから、Tier・リーグスターター・各ビルドの対応版を更新します。公開前の予想を最強ランキングとして掲載しません。</p></section>`;
const oneNextActions = `<section class="content-action"><h2>1.0から始める人が今やること</h2><ol><li><b>戦い方を決める：</b>近接・遠距離・スペル・ミニオンから、続けたい操作感を選ぶ。</li><li><b>現行環境で試す：</b>0.5.5の確認済みビルドで基本操作と育成の流れを体験する。</li><li><b>基本を覚える：</b>スキル・装備・耐性・パッシブの更新順を確認する。</li><li><b>1.0公開後に選び直す：</b>正式パッチノートと再確認済みビルドが公開されてから、1.0用ビルドを決める。</li></ol><p>1.0向けの最強ビルドはまだ確定していません。現時点では0.5.5の確認済み情報だけを使います。</p><div class="section-cta"><a class="button" href="/beginner-guide/">初心者向けビルドを見る</a><a class="button-secondary" href="/best-builds/">今使えるおすすめビルドを見る</a><a class="button-secondary" href="/league-starter/">リーグスターターを見る</a></div></section>`;
const oneContentWithActions = oneContent
  .replace(/<section class="content-action"><h2>初心者は何を選べばいい？<\/h2>[\s\S]*?<\/section>(?=<section><h2>1\.0を待たず)/, oneNextActions)
  .replace("<section><h2>更新履歴</h2><ul>", "<section><h2>更新履歴</h2><ul><li>2026-09-26：1.0から始める人向けの4段階アクションと主要導線を追加</li>");
await mkdir(resolve(root, "poe2-1-0"), { recursive: true });
await writeFile(resolve(root, "poe2-1-0/index.html"), shell({ title: "PoE2 1.0はいつ？2026年12月11日正式リリース・無料化", description: "PoE2 1.0は2026年12月11日正式リリース予定。基本プレイ無料化、Duelist、現行0.5.5ビルドとの違いを公式情報だけで整理します。", path: "/poe2-1-0/", current: "", kicker: "PATH OF EXILE 2 / VERSION 1.0", h1: "PoE2 1.0はいつ？正式リリース・無料化の確定情報", intro: "2026年12月11日の正式版・基本プレイ無料化に向け、確定情報と未確定のビルド情報を分けて更新します。", content: oneContentWithActions, crumbs: [{ name: "ホーム", path: "/" }, { name: "PoE2 1.0", path: "/poe2-1-0/" }], statusLabel: "現在掲載ビルド" }));
...
const publishedNoteRows = [
  ["Spirit Walker（ハントレス）ツイスタービルド完全ガイド", "https://note.com/fresh_okapi3816/n/nbe96101eda53", "/builds/huntress/twister-spirit-walker/", "huntress_twister_spirit_walker"],
  ["グレネード・ジェムリング解説", "https://note.com/fresh_okapi3816/n/nfd9d6bfbb0d8", "/builds/mercenary/grenade-gemling/", "mercenary_grenade_gemling"],
  ["0.5.5 Tier比較", "https://note.com/fresh_okapi3816/n/n5c499efd7391", "/tier-list/", "poe2_tier_055"],
  ["1.0正式版の速報", "https://note.com/fresh_okapi3816/n/n8e50e45e6c73", "/poe2-1-0/", "poe2_1_0_news"],
  ["1.0無料化・初心者向けクラス", "https://note.com/fresh_okapi3816/n/ne5263724eb0f", "/classes/", "poe2_1_0_beginner_classes"],
  ["0.5.5レベリング効率化", "https://note.com/fresh_okapi3816/n/nf6f8c2db6ed1", "/leveling/", "leveling_055"],
  ["装備の優先順位", "https://note.com/fresh_okapi3816/n/n63f105b8740d", "/guides/gear-upgrade/", "gear_priority"],
  ["マッピング最強ビルド3選", "https://note.com/fresh_okapi3816/n/nbd2ce44d8585", "/best-builds/", "mapping_best_builds"],
  ["低予算ビルド3選", "https://note.com/fresh_okapi3816/n/n09f92db3dc1a", "/best-builds/", "budget_best_builds"],
  ["ボスで詰まった時に見直す5項目", "https://note.com/fresh_okapi3816/n/nb6f6a2709e4a", "/guides/cant-beat-boss/", "cant_beat_boss"]
];
const noteMap = `# note / X → POE2ビルドナビ 送客対応表\n\n同じ本文を転載せず、noteでは特徴・選び方を説明し、サイトではLv1からの手順と現在Lv入力を提供します。トップではなく、投稿テーマに一致するURLへ直接送ります。\n\n## 公開済みnote記事からの優先リンク\n\n| note記事 | 記事URL | 最適なリンク先 | 計測用URL |\n|---|---|---|---|\n${publishedNoteRows.map(([theme, noteUrl, path, campaign]) => `| ${theme} | ${noteUrl} | ${path} | ${absolute(path)}?utm_source=note&utm_medium=referral&utm_campaign=${campaign} |`).join("\n")}\n\n## 公開済み記事の修正候補\n\n| 記事 | 残存箇所 | 差し替え先 |\n|---|---|---|\n| [1.0無料化・初心者向けクラス](https://note.com/fresh_okapi3816/n/ne5263724eb0f) | 「Ice Shot Deadeye 完全ロードマップ」の未設定URL | ${absolute("/builds/ranger/ice-shot-deadeye/")}?utm_source=note&utm_medium=referral&utm_campaign=poe2_1_0_ice_shot |\n| 同上 | 「Spirit Walker（ハントレス）ツイスター完全ガイド」の未設定URL | ${absolute("/builds/huntress/twister-spirit-walker/")}?utm_source=note&utm_medium=referral&utm_campaign=poe2_1_0_spirit_walker |\n| 同上 | 「ミニオン系初心者ビルドの選び方」の未設定URL | ${absolute("/builds/witch/minion-infernalist/")}?utm_source=note&utm_medium=referral&utm_campaign=poe2_1_0_minion |\n\n今回の公開検索で確認できたPoE2記事には、「有料記事」と誤解させる表記は見つかりませんでした。上の未設定URLは誤解や離脱につながるため、note側で差し替えが必要です。\n\nウィッチハンター比較・ストームウィーバー比較は、単一の育成手順と一致する個別ビルドがまだないため、無関係な個別ページへは送りません。\n\n## 投稿テーマ別リンク\n\n| 投稿テーマ | リンク先 | note用UTMリンク | X用UTMリンク |\n|---|---|---|---|\n${noteRows.map(([theme, path, campaign]) => `| ${theme} | ${path} | ${absolute(path)}?utm_source=note&utm_medium=referral&utm_campaign=${campaign} | ${absolute(path)}?utm_source=x&utm_medium=social&utm_campaign=${campaign} |`).join("\n")}\n\nCTA例：\n\n- Lv1から完成までの育成手順はこちら\n- 現在Lvから次にやることを見る\n- 装備・スキル・パッシブまでまとめて確認\n`;
