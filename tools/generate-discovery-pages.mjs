...
const bestContent = `<section class="content-action"><h2>結論：目的別のおすすめ候補</h2><p>「最強」は周回・ボス・防御・装備条件で変わります。掲載${builds.length}ビルドの確認済みデータから、目的ごとに1本ずつ候補を選びました。</p></section><div class="purpose-grid">${bestBuildCards}</div><section><h2>Tier・リーグスターターとの違い</h2><div class="pros-cons"><section class="info-card"><h3>初心者向けTier</h3><p>Lv1からの育てやすさ、操作、構成切替、資料確認状況を重視します。</p><a href="/tier-list/">初心者向けTierを見る</a></section><section class="info-card"><h3>リーグスターター</h3><p>資産が少ない序盤から育成できるか、装備依存やSSF情報を重視します。</p><a href="/league-starter/">リーグスターターを見る</a></section></div></section><section><h2>選定と更新の方針</h2><ul><li>掲載中かつLv1〜Endgameの主要情報を確認できたビルドだけを対象にします。</li><li>同条件の実測DPS順位は作らず、既存資料で確認できる用途・操作・装備条件を使います。</li><li>パッチ変更時は公式ノートと各ビルドの段階別資料を再確認します。</li></ul><p><a href="/editorial-policy/">編集方針と情報確認方法を見る</a></p></section><section class="related"><h2>ビルドを決めた後</h2><div class="related-links"><a href="/leveling/">別のビルド・現在Lvから探す</a><a href="/classes/">職業から選び直す</a><a href="/builds/">確認済み${builds.length}ビルドを見る</a></div></section>`;
...
const oneContent = `<section class="content-action"><h2>結論：PoE2 1.0は2026年12月11日公開予定</h2><ul>${one.confirmed.map((item) => `<li>${esc(item)}</li>`).join("")}</ul><p>日付・無料化・Duelistは公式発表で確認済みです。1.0環境のビルド性能はまだ確定していません。</p></section><section><h2>PoE2 1.0とは</h2><p>早期アクセスから正式版へ移行する大型更新です。このページでは公式発表で確認できた内容と、まだ断定できないビルド情報を分けます。</p></section><section><h2>正式リリース日は2026年12月11日</h2><p>公式発表では、Path of Exile 2の正式版1.0は2026年12月11日に公開予定です。</p><a class="button-secondary" href="/poe2-1-0/release-date/">公開予定日を詳しく見る</a></section><section><h2>基本プレイ無料化について</h2><p>正式版1.0の公開時に基本プレイ無料へ移行予定であることが公式発表されています。価格・対応機種など、このサイトのビルド選択と直接関係しない未確認情報は扱いません。</p><p><a href="${esc(one.sourceUrl)}" target="_blank" rel="noopener noreferrer">${esc(one.sourceName)}の正式版発表</a>（確認：${esc(one.checkedAt)}）</p></section><section><h2>新職業Duelist</h2><p>Duelistは正式版1.0で追加予定です。スキル・アセンダンシー・既存ビルドへの影響は、正式なパッチノートと段階別資料を確認できるまで推測で掲載しません。</p><div class="section-cta"><a class="button-secondary" href="/poe2-1-0/duelist/">Duelistの確認済み情報</a><a class="button-secondary" href="${esc(one.duelistSourceUrl)}" target="_blank" rel="noopener noreferrer">Duelistの公式発表</a></div></section><section class="content-action"><h2>1.0公開前に断定しない情報</h2><ul>${one.unconfirmed.map((item) => `<li>${esc(item)}</li>`).join("")}</ul></section><section><h2>1.0で現在の0.5.5ビルドは使える？</h2><p>変更なしで使えるとは断定できません。現在掲載中の${builds.length}ビルドは0.5.5向けで、正式版公開後に公式変更と各ビルドの育成資料を再確認してから1.0対応表示へ切り替えます。</p><div class="tool-grid"><a class="tool-card" href="/best-builds/"><span>現行0.5.5</span><strong>目的別おすすめ</strong><p>現在確認済みの候補だけを比較します。</p></a><a class="tool-card" href="/tier-list/"><span>BEGINNER TIER</span><strong>初心者向けTier</strong><p>育てやすさと構成切替で比較します。</p></a><a class="tool-card" href="/league-starter/"><span>STARTER</span><strong>リーグ開始候補</strong><p>低資産・序盤条件から選びます。</p></a></div></section><section id="one-build-impact"><h2>現在のビルドへの影響</h2><p>掲載中の${builds.length}ビルドは0.5.5向けです。1.0向けに再確認済みのビルドはまだありません。公式パッチノートと段階別資料を確認するまで、0.5.5の手順を1.0対応とは表示しません。</p></section><section><h2>よくある質問</h2><details><summary>PoE2 1.0はいつ？</summary><p>2026年12月11日に公開予定です。</p></details><details><summary>正式版は無料？</summary><p>公式発表では、1.0公開時に基本プレイ無料へ移行予定です。</p></details><details><summary>0.5.5のビルドを1.0でもそのまま使える？</summary><p>まだ確認できません。正式な変更内容と段階別資料を確認してから更新します。</p></details></section><section><h2>更新履歴</h2><ul><li>2026-09-24：検索意図を整理し、確定情報・未確定情報・FAQを分離</li><li>2026-09-23：正式版・基本プレイ無料化・Duelistの公式情報を更新</li></ul></section><section><h2>1.0公開後の更新方針</h2><p>公式パッチノート、スキル変更、アセンダンシー、段階別の書面ガイドを再確認してから、Tier・リーグスターター・各ビルドの対応版を更新します。公開前の予想を最強ランキングとして掲載しません。</p></section>`;
...
const publishedNoteRows = [
  ...
  ["1.0正式版の確認済み情報", "https://note.com/fresh_okapi3816/n/ne5263724eb0f", "/poe2-1-0/", "poe2_1_0_confirmed"]
];
