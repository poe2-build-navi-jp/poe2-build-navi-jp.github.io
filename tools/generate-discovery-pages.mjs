...
const levelingContent = `<section class="content-action"><h2>現在Lvから探す</h2><p>職業と使っているビルド、現在Lvを選ぶと、既存の育成ページでスキル・装備・パッシブの「今やること」を3つに絞って表示します。</p><form id="leveling-form" class="leveling-form"><label>職業<select id="leveling-class">${levelingClassOptions}</select></label><label>ビルド<select id="leveling-build">${levelingBuildOptions}</select></label><label>現在Lv<input id="leveling-level" type="number" min="1" max="100" value="1" inputmode="numeric"></label><a id="leveling-cta" class="button" href="/builds/monk/whirling-assault/?level=1#now">現在Lvから今やることを見る</a></form><div class="leveling-stages" aria-label="レベル帯から選ぶ">${levelingStages}</div></section><section><h2>使っているビルドから選ぶ</h2><p>共通説明を重複させず、各ビルドにあるLv別の確認済み育成手順へ直接案内します。</p><div class="seo-build-list">${levelingBuildCards}</div></section><section class="related"><h2>まだビルドが決まっていない場合</h2><div class="related-links"><a href="/classes/">おすすめクラスから選ぶ</a><a href="/beginner-guide/">初心者向けビルドから選ぶ</a><a href="/league-starter/">序盤スターターから選ぶ</a></div></section><script type="module" src="/assets/leveling.js?v=20260919-1"></script>`;
